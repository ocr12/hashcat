...
static const char *HASH_NAME      = "MIKROTIK EC-SRP5";
...
{
  return sizeof(precomputed_inputs_t);
}

DECLSPEC void print_num(const char* prefix, const u32* n)
{
  printf("\n%s: ", prefix);
  for (int i = 7; i >= 0; --i)
   printf("%x ", n[i]);
...
int module_hash_decode (MAYBE_UNUSED const hashconfig_t *hashconfig, MAYBE_UNUSED void *digest_buf, MAYBE_UNUSED salt_t *salt, MAYBE_UNUSED void *esalt_buf, MAYBE_UNUSED void *hook_salt_buf, MAYBE_UNUSED hashinfo_t *hash_info, const char *line_buf, MAYBE_UNUSED const int line_len)
{
  ...
  /* Precompute a user string as a SHA256 context */
  precomputed_inputs_t *esalt = (precomputed_inputs_t *)esalt_buf;
  const u8 *user = token.buf[0];
  size_t user_len = token.len[0];
  u32 block[16] = { 0 }; /* SHA256 block must be zero-padded */
  sha256_init(&esalt->user_ctx);
  for (size_t i = 0; i < user_len; ++i)
  {
    block[0] = byte_swap_32(user[i]);
    sha256_update(&esalt->user_ctx, block, 1);
  }
  /* Add a colon */
  {
    block[0] = byte_swap_32(':');
    sha256_update(&esalt->user_ctx, block, 1);
  }

  /* Precompute a salt string as a SHA256 context */
  sha256_init(&esalt->salt_ctx);
  const u8 *hex_salt = token.buf[1];
  for (size_t i = 0; i < 4; ++i)
  {
    block[0] = byte_swap_32(hex_to_u32(hex_salt + i * 8));
    sha256_update(&esalt->salt_ctx, block, 4);
  }

  if (!generic_salt_decode (hashconfig, token.buf[1], token.len[1], (u8 *) salt->salt_buf, (int *) &salt->salt_len))
  {
    return (PARSER_SALT_LENGTH);
  }

  /* Convert and set a hex digest */  
